        ...
        loop {
            ...
            return match event {
                ...
                Event::Start(bytes_start) => Ok(GenericXmlEvent::StartElement {
                    name: GenericXmlName { local_name: self.decode(bytes_start.local_name()).to_string() },
                    attributes: bytes_start.attributes()
                        .map(|x| x.unwrap())
                        .map(|x| GenericXmlAttribute {
                            name: GenericXmlName { local_name: self.decode(x.key).to_string() },
                            value: x.unescape_and_decode_value(self).unwrap(),
                        }).collect(),
                }),
                Event::End(bytes_end) => Ok(GenericXmlEvent::EndElement {
                    name: GenericXmlName { local_name: self.decode(bytes_end.local_name()).to_string() },
                }),
